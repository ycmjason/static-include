...
Rule.useTemplate = function(name, values){
  var template_path;
  switch(name){
    case 'javascript':
    case 'js':
      template_path = '../rule_templates/javascript.js';
      break;
    case 'css':
    case 'stylesheet':
      template_path = '../rule_templates/css.js';
      break;
    default:
      throw new Error(RULE_TEMPLATE_NAME_NOT_FOUND);
  }

  return {
    withValues: (values) => {
      var rule = require(template_path);
      rule.values = values;
